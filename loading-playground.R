...
# NEO_GenericLogName_20240606-110843.txt
# extract the last number-number part of the file name
...
find_session_dates(folder)

session <- load_session(folder, codes[1])
head(session$position)

head(cyberframer::unity_vector_to_numeric(session$position$position))
