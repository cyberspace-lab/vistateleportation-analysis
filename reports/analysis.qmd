...
df_questionnaires <- df_data %>%
  select(participant, starts_with("ssq_"), starts_with("vrsq_"), starts_with("vrleq_"), Movement) %>%
  distinct() 
...
```{r lmer-relative-distance-predictions}
...
glm_distance <- glm((distance_items + distance_return) ~ Movement + Vista + LevelSize + Movement:Vista + Movement:LevelSize,
